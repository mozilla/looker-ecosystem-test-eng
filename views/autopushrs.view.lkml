    ...
    sql:
      WITH autopushrs_averages AS (
        ...
        (SELECT AVG(sub_r.`Execution Time`)
         FROM `test_metrics.autopushrs_suite_results` sub_r
        ...
        FROM `test_metrics.autopushrs_suite_results` r
      ...
      FROM `test_metrics.autopushrs_suite_results` r
      LEFT JOIN autopushrs_averages a
