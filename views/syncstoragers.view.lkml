view: syncstoragers {
  derived_table: {
    sql:
      WITH syncstoragers_averages AS (
        SELECT
      ...
      FROM `test_metrics.syncstoragers_suite_results` sub_r
      ...
      FROM `test_metrics.syncstoragers_suite_results` r
  ...
  dimension: is_last_quarter_date {
    ...
    sql: ${timestamp_raw} = (
          ...
          FROM `test_metrics.syncstoragers_suite_results`
          WHERE
            `Workflow` = ${workflow}
